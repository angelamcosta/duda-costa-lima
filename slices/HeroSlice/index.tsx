    ...
    <span aria-label={text}>
      {chars.map((c, i) => (
        <span
          ...
          style={{ "--char-delay": `${baseDelay + i * perChar}ms` } as React.CSSProperties}
        >
          {c === " " ? " " : c}
        </span>
      ))}
...
interface GlyphItem {
  ...
  vx: number; vy: number; size: number; ch: string
  rot: number; tone: number; alpha: number
}
...
function HeroBackdrop() {
  ...
  useEffect(() => {
    ...
    const PUSH_RADIUS = 240, PUSH_FORCE = 0.6, SPRING_K = 0.025, DAMPING = 0.86
    let raf: number
      ...
      for (const it of items) {
        if (mouse.active) {
          ...
          if (d < PUSH_RADIUS && d > 0.001) {
            ...
            it.vx += (dx / d) * f; it.vy += (dy / d) * f
          }
        }
        it.vx += (it.hx - it.x) * SPRING_K
        it.vy += (it.hy - it.y) * SPRING_K
        it.vx *= DAMPING; it.vy *= DAMPING
        ...
        const rgb = it.tone < 0.45 ? palette.jet
          : it.tone < 0.70 ? palette.warm
          : it.tone < 0.90 ? palette.soft
          : palette.deep
    ...
    function onResize() { palette = readPalette(); init() }
    function onLeave() { mouse.active = false; wrap!.style.setProperty("--spot-on", "0") }
...
export function HeroSlice({ primary, lang }: Props) {
  ...
  return (
    <section className="hero frame" id="hero">
      ...
      <h1 className="hero-name" aria-label={`${primary.name_a} ${primary.name_b}`}>
        <span className="row">
